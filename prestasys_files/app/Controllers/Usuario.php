...
class Usuario extends ResourceController
{
	...
	public function show($id = null)
	{
		...
		$us = (new Usuario_model())
			->select("regnro, ruc, dv, tipoplan, email, cliente, cedula, 
				...
				saldo_IVA, ultimo_nro, clave_marangatu, timbrado ")->where("regnro", $id)->first();
	...
	public function ruc($ruc = null)
	{
		...
		$us = (new Usuario_model())
			->select("regnro, ruc, dv, tipoplan, email, cliente, cedula, telefono, celular, domicilio, ciudad, rubro,
				timbrado")->where("concat(ruc,dv)", $ruc)->first();
		...
		if ($this->validate('usuarios')) {
			...
			try {
				...
				$usuario_Response = (new Usuario_model())
					->select("regnro, ruc, dv, tipoplan, email, cliente, cedula, telefono, celular, domicilio, ciudad, rubro, saldo_IVA")->where("regnro", $id)->first();
...
*/
					...
					$usu_Response = (new Usuario_model())
						->select("regnro, ruc, dv, tipoplan, email, cliente, cedula, telefono, celular, domicilio, ciudad, rubro, saldo_IVA")->where("regnro",  $data['regnro'])->first();
...
*/
	...
	//Respecto al cierre de mes de TODOS los usuarios
...
IF( (select estado_mes.regnro from estado_mes where estado_mes.codcliente= usuarios.regnro and estado_mes.estado<>'L' and estado_mes.estado<>'P' order by created_at desc limit 1) IS NULL, 0, 1) AS novedad_c_mes,
...
(select estado_mes.mes from estado_mes where estado_mes.codcliente= usuarios.regnro and estado_mes.estado<>'L' and estado_mes.estado<>'P'   order by created_at desc limit 1) AS c_mes,
(select estado_mes.anio from estado_mes where estado_mes.codcliente= usuarios.regnro and estado_mes.estado<>'L' and estado_mes.estado<>'P'   order by created_at desc limit 1) AS c_anio 
FROM `usuarios` 
			";
